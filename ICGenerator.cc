...
void ICGenerator::optimize_tree(Node *parent, SyntaxTree *tree) {
  Node *n = NULL;
  Origin origin;
  if(parent->GetChild() != NULL)
    optimize_tree(parent->GetChild(), tree);
  else if(parent->GetLeftChild() != NULL)
    optimize_tree(parent->GetLeftChild(), tree);  
  if(parent->GetRightChild() != NULL)
    optimize_tree(parent->GetRightChild(), tree);


  for(int i = 0; i < 3; i++){
    origin = (Origin) i;
    if(origin == LEFT && parent->GetLeftChild() != NULL)
      n = parent->GetLeftChild();
    if(origin == RIGHT && parent->GetRightChild() != NULL)
      n = parent->GetRightChild();
    if(origin == MIDDLE && parent->GetChild() != NULL)
      n = parent->GetChild();
    if(n == NULL)
      return;
    ...
    switch(n->GetNodeType()){
      case NODE_ADD:
        cerr << "NODE ADD" << endl;
        if(n->GetLeftChild()->GetNodeType() == NODE_NUM_INT
        && n->GetRightChild()->GetNodeType() == NODE_NUM_INT){
          cerr << "NODE ADD DELETE" << endl;
          Node* new_node = new Node_Integer();
          new_node->SetNodeType(NODE_NUM_INT);
          new_node->SetReturnType(RT_INT);
          int value = n->GetLeftChild()->GetIntValue() + n->GetRightChild()->GetIntValue();
          new_node->SetIntValue(value);
          if(origin == LEFT)
...
void ICGenerator::Preprocess (SyntaxTree * tree, SymbolTable * symtab) {
  Node *n = tree->GetProgramBody();
  optimize_tree(n, tree);
  for (int i = 0; i < tree->GetSubprogramCount(); i++) {
    n = tree->GetSubprogram(i);
    optimize_tree(n, tree);
  }
}
